...
export function generateStaticParams() {
  return Object.keys(landingpageData).map((slug) => ({
    slug,
  }));
}

export default async function LandingPageDetailsPage({ params }) {
  const { slug } = await params;
  const project = landingpageData[slug];
      ...
      <section className="w-full px-6 sm:px-10 lg:px-16 flex flex-col lg:flex-row items-start justify-between gap-8 lg:gap-12 mt-12 sm:mt-16 lg:mt-24 mb-6">
        ...
        <div className="w-full lg:w-[42%] flex flex-col justify-between flex-shrink-0 pt-0">
          <div className="space-y-8 sm:space-y-10">
            {/* Header & Back Link pointing to /landing-page */}
            <div className="flex items-center justify-between">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-medium tracking-tight text-zinc-950">
                {project.title}
              </h1>
              <Link
                href="/landing-page"
                className="flex items-center gap-1.5 text-xs sm:text-sm text-zinc-400 hover:text-zinc-950 transition-colors font-medium group"
              >
            ...
            <div className="grid grid-cols-2 gap-8">
              ...
              <div className="space-y-1.5">
                ...
                <a
                  ...
                  className="text-xs sm:text-sm lg:text-base text-zinc-950 font-medium hover:text-zinc-500 transition-colors inline-flex items-center gap-1.5 group"
                >
                  <span className="text-zinc-400 group-hover:translate-x-0.5 transition-transform">↪</span>
                  <span className="underline underline-offset-4">{project.liveLinkDisplay || "Visit Website"}</span>
                </a>
              </div>
            </div>
